...
VIDEOS_PARA_GERAR = [
    {
        ...
        'mascara': 'assets/backgrounds/fundoaltosemtexto.png',  # Máscara será redimensionada automaticamente
        'mascara2': 'assets/backgrounds/fundoalto.png',
    ...
    },
    {
        ...
        'mascara': 'assets/backgrounds/fundobaixosemtexto.png',  # Máscara específica para resolução alternativa (1680x1176)
        'mascara2': 'assets/backgrounds/fundobaixo.png',
...
def carregar_e_redimensionar(caminho_imagem, largura, altura):
    ...
    img_rgb = carregar_e_redimensionar_com_deteccao_rosto(caminho_imagem, largura, altura, verbose=True)
    
    # Aplica filtro vermelho (Red tint)
    img_float = img_rgb.astype(np.float32)
    # Aumenta canal vermelho e reduz os outros para dar um tom vermelho estilizado
    img_float[:, :, 0] = np.clip(img_float[:, :, 0] * 1.5 + 40, 0, 255) # Red
    img_float[:, :, 1] = img_float[:, :, 1] * 0.4 # Green
    img_float[:, :, 2] = img_float[:, :, 2] * 0.4 # Blue
...
def calcular_posicao_origem(x_final, y_final, largura_foto, altura_foto, largura_video, altura_video, direcao, escala_inicial=1.0):
    ...
    """
    # Para criar um efeito de preview, todas as fotos aparecem no centro da tela e diminuem para seus lugares
    largura_real = int(largura_foto * escala_inicial)
    altura_real = int(altura_foto * escala_inicial)
    
    # Retorna o centro absoluto da tela
    return (largura_video // 2 - largura_real // 2, altura_video // 2 - altura_real // 2)
