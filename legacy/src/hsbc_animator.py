...
def generate_hsbc_video(main_img_path: Path, base_images_paths: list[Path], out_video_path: Path, fill_internal: bool = False):
    ...
    if fill_internal:
        ...
        for r, bounds in row_bounds.items():
            ...
            for c in range(min_c, max_c + 1):
                # Guarda tupla (col, row, is_red)
                final_cells.append((c, r, (c, r) in red_cells))
    else:
        for c, r in red_cells:
            final_cells.append((c, r, True))

    total_cells = len(final_cells)
    random.seed(42)
    random.shuffle(final_cells)
    ...
    # Pre-calcular as imagens da galeria (red-tinted ou normais)
    precalculated_tiles = []
    for (c, r, is_red) in final_cells:
        ...
        if is_red:
            ...
            precalculated_tiles.append(img_bgr)
    ...
    for f_idx in range(total_frames):
        ...
        for idx, (c, r, is_red) in enumerate(final_cells):
