...
def _obter_cliente_s3(log_callback=None):
    ...
    try:
        ...
        if log_callback:
            log_callback(f"❌ Falha ao inicializar cliente S3: {exc}")
        return None, None

def testar_acesso_s3(log_callback=None):
    if log_callback:
        log_callback("🔄 Testando acesso ao S3 para download...")
    s3_client, bucket_name = _obter_cliente_s3(log_callback)
    if not s3_client:
        if log_callback:
            log_callback("⚠️ S3 não configurado (verifique .env). Downloads do bucket ignorados.")
        return False
    try:
        response = s3_client.list_objects_v2(Bucket=bucket_name, MaxKeys=1)
        if log_callback:
            log_callback("✅ Teste S3 concluído com sucesso. Permissões validadas!")
        return True
    except Exception as exc:
        if log_callback:
            log_callback(f"❌ Erro no teste do S3 (verifique permissões e credenciais): {exc}")
        return False

PROCESSADAS_S3 = set()

def baixar_novas_imagens_s3(pasta_entrada: Path, log_callback=None):
    s3_client, bucket_name = _obter_cliente_s3(log_callback)
    if not s3_client:
        return
    ...
    try:
        ...
        for obj in objetos:
            chave = obj["Key"]
            ext = Path(chave).suffix.lower()
            if ext in EXTENSOES_SUPORTADAS and chave not in PROCESSADAS_S3:
                destino = pasta_entrada / Path(chave).name
                if not destino.exists():
                    if log_callback:
                        log_callback(f"☁️ Baixando nova imagem do S3: {chave}")
                    s3_client.download_file(bucket_name, chave, str(destino))
                PROCESSADAS_S3.add(chave)
                
    except Exception as exc:
        if log_callback:
            log_callback(f"❌ Erro ao verificar novas imagens no S3: {exc}")
...
def processar_imagem(
    ...
    with Image.open(caminho_imagem) as raw:
        ...
        if filtro_vermelho:
            r, g, b = img.split()
            g = g.point(lambda i: int(i * 0.15))
            b = b.point(lambda i: int(i * 0.15))
    ...
    for caminho in ficheiros:
        ...
        try:
            destino_mosaic = processar_imagem(
                ...
                filtro_vermelho=filtro_vermelho,
            )
            proximo_indice += 1
            ok += 1
...
def monitorar_e_gerar(
    ...
    log(f"🔥 Filtro vermelho: {'Sim' if filtro_vermelho else 'Nao'}")
    ...
    try:
        testar_acesso_s3(log_callback=log)
        while not stop_signal.is_set():
            baixar_novas_imagens_s3(pasta_entrada_path, log_callback=log)
